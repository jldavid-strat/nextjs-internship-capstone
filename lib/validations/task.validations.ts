...
export const BaseTaskSchema = z
  ...
  .superRefine((data, ctx) => {
    ...
    // Ensure startDate <= dueDate (ignoring time part)
    const startDay = new Date(
      startDateD.getFullYear(),
      startDateD.getMonth(),
      startDateD.getDate(),
    );
    const dueDay = new Date(dueDateD.getFullYear(), dueDateD.getMonth(), dueDateD.getDate());

    const nowTime = new Date().getTime();

    if (startDay.getTime() > dueDay.getTime()) {
      ctx.addIssue({
        path: ['startDate'],
        code: 'too_big',
        maximum: nowTime,
